...
async function fetchYear(year: number): Promise<ApiYearResponse> {
  ...
  return res.json() as Promise<ApiYearResponse>
}
...
export default function GainsAnnuelsPage() {
  ...
  React.useEffect(() => {
    ...
    const run = async () => {
      setIsLoading(true)
      try {
        const [y, p] = await Promise.all([fetchYear(selectedYear), fetchYear(selectedYear - 1)])
        if (cancelled) return
        setDataYear(y)
        setDataPrev(p)
      } catch {
  ...
  const totals = React.useMemo(() => {
    ...
    const avgNonZero = nonZero.length ? nonZero.reduce((s, m) => s + m.amount, 0) / nonZero.length : 0
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    <Card className="pt-0">
                      ...
                      <CardContent className="pt-0">
                        <div className="text-xs text-muted-foreground">
                          Moyenne (12 mois) : <span className="font-medium text-foreground">{formatEUR(totals.avg)}</span>
                        </div>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="pt-0">
                      ...
                      <CardContent className="pt-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-muted-foreground">Total N-1 : {formatEUR(totals.prev)}</span>
                        </div>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="pt-0">
                      <CardHeader className="pb-2">
                        ...
                        <CardTitle className="text-2xl">{MONTHS_SHORT_FR[totals.best.monthIndex]}</CardTitle>
                      </CardHeader>
                      <CardContent className="pt-0">
                        <div className="text-xs text-muted-foreground">{formatEUR(totals.best.amount)}</div>
                      </CardContent>
                    </Card>
                      ...
                      <CardContent className="pt-0">
                        <div className="flex items-center gap-2">
                          <Badge variant={(totals.momDelta ?? 0) >= 0 ? "default" : "destructive"}>
                            {totals.momPct === null ? "N/A" : `${totals.momPct >= 0 ? "+" : ""}${totals.momPct.toFixed(1)}%`}
                          </Badge>
                          <span className="text-xs text-muted-foreground">Mois à zéro : {totals.monthsWithZero}</span>
                        </div>
                      </CardContent>
                    </Card>
                  </div>
                  ...
                  <ChartContainer config={chartConfig} className="aspect-auto h-[260px] w-full">
                    <AreaChart data={filteredChartData}>
                          ...
                          content={
                            <ChartTooltipContent
                                ...
                                formatter={(value: ValueType, name: NameType) => {
                                  ...
                                  const n = String(name)
                                  const label = n === "year" ? String(selectedYear) : String(selectedYear - 1)
                      ...
                      <Area dataKey="prev" type="natural" fill="url(#fillPrev)" stroke="var(--color-prev)" name="prev" />
                      <Area dataKey="year" type="natural" fill="url(#fillYear)" stroke="var(--color-year)" name="year" />
                  ...
                  <div className="text-xs text-muted-foreground">
                    Calcul basé sur tes temps saisis :{" "}
                    <span className="font-medium text-foreground">montant = (TJM / 450) × minutes</span>. Moyenne sur mois
                    travaillés : <span className="font-medium text-foreground">{formatEUR(totals.avgNonZero)}</span>.
                  </div>
                </>
            )}
          </CardContent>
